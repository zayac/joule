...
let merge_bounds depth old_terms new_terms =
  ...
  if Logic.Map.is_empty old_terms && Logic.Map.is_empty new_terms then
    ...
    Log.logf "%smerged upper bounds {%s} and {%s} into {%s}"
  ...
  String.Map.iter2 left right ~f:(fun ~key ~data ->
    let add l t =
      List.map !result ~f:(fun (logic, map) ->
        Logic.(l * logic), String.Map.add map ~key ~data:(l, t)) in
      match data with
      | `Left (l, t)
      | `Right (l, t) ->
        result := add l t
...
let poly_var_to_list context var logic_constr =
  match var with
  | None -> context, []
  | Some v ->
...
let rec solve_senior depth context left right =
  ...
  try
    Log.logf "%ssolving constraint %s <= %s" (String.make depth ' ')
    ...
    | Tuple t, Nil ->
        List.fold ~init:context ~f:(fun acc el ->
          ...
          solve_senior (depth + 1) acc left right) t
    | Nil, Tuple t ->
        List.fold ~init:context ~f:(fun acc el ->
    ...
    | List (t, var), Nil ->
      begin
        let context = List.fold ~init:context ~f:(fun acc el ->
          let left = logic_left, el in
          solve_senior (depth + 1) acc left right) t in
        match var with
        | None -> context
        | Some v ->
    ...
    | List (t, var), List (t', var') ->
      ...
      | [], Some v, _ ->
        ...
        let context, var_list =
          poly_var_to_list context var' logic_right in
        let merged = ref [] in
    ...
    | Record (map, var), Nil ->
      begin
        let context = String.Map.fold map ~init:context
          ~f:(fun ~key ~data acc ->
          let l, t = data in
          solve_senior (depth + 1) acc (l, t) right) in
        match var with
        | None -> context
        | Some v ->
          solve_senior (depth + 1) context (logic_left, Var v) right
    ...
    | Nil, Record (map, var) ->
      begin
        let context = String.Map.fold map ~init:context
          ~f:(fun ~key ~data acc ->
          let l, t = data in
          solve_senior (depth + 1) acc left (l, t)) in
        match var with
        | None -> context
        | Some v ->
          solve_senior (depth + 1) context left (logic_left, Var v)
    ...
    | Record (r, v), Record (r', v') ->
      begin
        ...
        let context = String.Map.fold r ~init:context
          ~f:(fun ~key ~data acc ->
            let (guard, term) = data in solve_senior (depth + 1) acc
              (Logic.(guard * logic_left), term)
              (logic_right, Nil)) in
            ...
            String.Map.iter r' ~f:(fun ~key ~data ->
              ...
              | Some (guard', term') ->
                try
                  ...
                  let logic = Logic.Set.add logic Logic.(~- guard + guard') in
                  ctx := constrs, logic
...
let resolve_bound_constraints topo =
  Log.logf "setting least upper bounds for constraints";
  ...
  while not !fixed_point do
    Log.logf "traversal #%d" !iter_counter;
    let ctx' = ref !ctx in
    List.iter ~f:(apply ctx) topo;
    (*let constrs, _ = !ctx in Constr.print_constraints constrs;*)
...
let solve_exn lst =
  ...
  try
    ...
    | None -> None
    | Some bool_map -> Some (Constr.substitute constrs bool_map)
